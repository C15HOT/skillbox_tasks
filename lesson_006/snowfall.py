...
def number_of_snowflake():
    # TODO ещё del_list стоит обновлять в начале этой функции, чтобы старые индексы в нём не оставались
    for num, coord_y in enumerate(pure_points):
        if coord_y[1] < 50:
            del_list.append(num)

            # так он всё ещё внутри цикла. это определяется отступом
    return num  # TODO обратите внимание, что возвращаете вы num - а что в нём хранится?
    # TODO а что должно возвращаться?
