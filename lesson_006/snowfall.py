...
def draw_snowflake(color=sd.COLOR_WHITE):
    ...
    # TODO нет, я написал пример, вы можете использовать цикл по списку points
    # TODO в целом можно даже без enumerate обойтись будет
    for i in range(N):
...
def number_of_snowflake():
    for num, coord_y in enumerate(pure_points):
        if coord_y[1] < 50:
            ...
            # TODO так он всё ещё внутри цикла. это определяется отступом
            return num  # ретурн здесь прервёт выполнение цикла и функции
    #  нужно вынести его из цикла
    # TODO вот тут начинается код, который будет выполнен после цикла
